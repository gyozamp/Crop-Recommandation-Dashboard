...
export default function Sidebar() {
  ...
  return (
    ...
    >
      ...
      {/* --- 1. Toggle Button (Il bottoncino rotondo sul bordo) --- */}
      <button
      ...
      {/* --- 2. Logo Section --- */}
      <div className={`flex items-center gap-3 mb-8 px-2 ${isExpanded ? "" : "justify-center"}`}>
        <div className="text-indigo-600 min-w-fit">
           <BoltIcon className="h-8 w-8" />
        </div>
        
        {/* Nascondiamo il testo con overflow-hidden e opacità per animazione fluida */}
      ...
      {/* --- 3. Navigation (Scrollabile se ci sono troppe voci) --- */}
      <nav className="flex flex-col gap-2 flex-1 overflow-y-auto scrollbar-hide">
        {navItems.map((item) => (
          ...
          >
            {/* Icona */}
            <item.icon className="h-6 w-6 min-w-[24px] transition-transform group-hover:scale-110" />
            
            {/* Label (Visibile solo se espanso) */}
            ...
            {/* Tooltip on Hover (Visibile solo se CHIUSO) */}
            {!isExpanded && (
      ...
      {/* --- 4. Footer / Logout Section (Sempre in basso) --- */}
      <div className={`mt-4 border-t border-gray-100 pt-4 ${isExpanded ? "" : "flex justify-center"}`}>
